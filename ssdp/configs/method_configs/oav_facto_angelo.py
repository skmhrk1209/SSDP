...
from ssdp.fields import OaVConfig
from ssdp.models import NeuSFactoAngeloConfig
...
config = MethodSpecification(
    config=_create_trainer_config(
        method_name="oav-facto-angelo",
        source_trainer_config=method_configs["neus-facto"],
    ),
    description="OaV-Facto-Angelo",
)
...
for suffix, scale_factor in dict(
    ...
    globals()[f"config_{suffix}"] = MethodSpecification(
        config=_create_trainer_config(
            method_name=f"oav-facto-angelo-{suffix}",
            source_trainer_config=method_configs["neus-facto"],
            scale_factor=scale_factor,
        ),
        description=f"OaV-Facto-Angelo with x{scale_factor} iterations",
    )
